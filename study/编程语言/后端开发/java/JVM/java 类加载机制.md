类从被加载到虚拟机内存中开始，到卸载出内存为止，它的整个生命周期包含以下7个阶段：
...
准备、验证、解析3个阶段统称为连接（Linking）

在类加载过程中加载、验证、准备、初始化和卸载这5个阶段的顺序是确定的，类加载过程必须按照这种顺序执行。

解析阶段是不一定的，在某些情况下可以在初始化阶段之后开始（为了支持java运行时绑定或者叫做动态绑定）
```

![](http://speed-up.cdn.ttxxly.top/20160308184325593.jpg)
